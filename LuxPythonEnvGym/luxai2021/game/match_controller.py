...
from ..game.actions import (
    SpawnCityAction, MoveAction, SpawnWorkerAction, SpawnCartAction
    )
...
class MatchController:
    ...
    def take_action(self, action):
        ...
        if action is not None:
            ...
            # Validate the action
            try:
                is_valid = action.is_valid(self.game, self.action_buffer, self.accumulated_stats)
                if is_valid:
                    # Add the action
        ...
        if actionable is not None:
            if (
                isinstance(action, SpawnCityAction)
                or isinstance(action, SpawnWorkerAction)
                or isinstance(action, SpawnCartAction)
            ):
                if actionable.can_act() and not is_valid:
                    actionable.set_can_act_override(True)
                else:
