...
def make_batch(episodes, args):
    ...
    for ep in episodes:
        ...
        for mot in moments:
            obs_zeros = map_r(mot['observation'][mot['turn'][0]], lambda o: np.zeros_like(o))  # template for padding
            p_zeros = np.zeros_like(mot['policy'][mot['turn'][0]])  # template for padding
            if (obs_zeros is not None) and (p_zeros.dtype != object):
                break
        ...
        if args['turn_based_training'] and not args['observation']:
            ...
            # (T, P, ALL_P) or (T, P, Unit, ALL_P)
            p = np.array([[replace_none(m['policy'][player], p_zeros) for player in players] for m in moments])
            # (T, P, 1) or (T, P, Unit, ALL_P)
            act = np.array([[replace_none(m['action'][player], p_zeros[..., 0]) for player in players] for m in moments], dtype=np.int64)[..., np.newaxis]
            # (T, P, ALL_P) or (T, P, Unit, ALL_P)
        ...
        if len(tmask) < args['forward_steps']:
            ...
            action_pad = [(0, pad_len), (0, 0), (0, 0)]
            if p.ndim == 4:
                action_pad += [(0, 0)]
            p = np.pad(p, action_pad, 'constant', constant_values=0)
            v = np.concatenate([v, np.tile(oc, [pad_len, 1, 1])])
            act = np.pad(act, action_pad, 'constant', constant_values=0)
            ...
            amask = np.pad(amask, action_pad, 'constant', constant_values=1e32)
...
def forward_prediction(model, hidden, batch, args):
    ...
    for k, o in outputs.items():
        # batch, step and player split with turn mask
        # policy (BxT, Unit, P_ALL) -> (B, T, P, Unit, P_ALL)
        # value (BxT, 1) -> (B, T, P, 1)
        turn_mask = batch["turn_mask"]
        if o.ndim == 2:
            o = o.view(*turn_mask.size()[:2], -1, o.size(-1))
        elif o.ndim == 3:
            o = o.view(*turn_mask.size()[:2], -1, *o.size()[-2:])
            # append Unit dimension
            turn_mask = turn_mask.unsqueeze(-2)

        if k == 'policy':
            # gather turn player's policies
            outputs[k] = o.mul(turn_mask).sum(2, keepdim=True) - batch['action_mask']
        else:
...
def compose_losses(outputs, log_selected_policies, total_advantages, targets, batch, args, sequence_mask=None):
    ...
    entropy = dist.Categorical(logits=outputs['policy']).entropy()
    if sequence_mask is not None:
        total_len = sequence_mask.sum(-2)
        total_len = torch.where(total_len == 0, 1, total_len)
        entropy = entropy.mul(sequence_mask.squeeze(-1)).sum(-1) / total_len.squeeze(-1)
    entropy = entropy.mul(tmasks.sum(-1))
...
def compute_loss(batch, model, hidden, args):
    ...
    if outputs["policy"].ndim == 4:
        emasks = batch['episode_mask']
    elif outputs["policy"].ndim == 5:
        # append unit seq dim
        emasks = batch['episode_mask'].unsqueeze(-2)
    ...
    sequence_mask = batch["observation"].get("sequence_mask", None)

    if sequence_mask is not None:
        # sum at log proba -> multiple product
        log_selected_t_policies = (log_selected_t_policies * sequence_mask).sum(-2)
        log_selected_b_policies = (log_selected_b_policies * sequence_mask).sum(-2)
    ...
    # if sequence_mask is not None:
    #     # sum over unit sequence
    #     total_len = sequence_mask.sum(-2)
    #     total_len = torch.where(total_len == 0, 1, total_len)

    #     log_selected_t_policies = (log_selected_t_policies * sequence_mask).sum(-2) / total_len
    #     cs = (cs * sequence_mask).sum(-2) / total_len
    #     clipped_rhos = (clipped_rhos * sequence_mask).sum(-2) / total_len
    ...
    return compose_losses(outputs, log_selected_t_policies, total_advantages, targets, batch, args, sequence_mask=sequence_mask)
